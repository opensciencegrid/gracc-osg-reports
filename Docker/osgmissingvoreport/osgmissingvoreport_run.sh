...
# Wrapper script to run the OSG Missing VO report inside a Docker container
# Example:  ./osgmissingvoreport_run.sh weekly


export VERSIONRELEASE=2.0.2
export TOPDIR=/opt/gracc-osg-reports
export LOCALLOGDIR=${TOPDIR}/log
export SCRIPTLOGFILE=${LOCALLOGDIR}/missingvo_run.log
export REPORTLOGFILE=${LOCALLOGDIR}/osgreporter.log
export CONFIGDIR=${TOPDIR}/config

function usage {
    echo "Usage:    ./osgmissingvoreport_run.sh [-p] <time period>"
    echo ""
